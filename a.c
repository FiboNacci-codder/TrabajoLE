...
// arreglo[]: El arreglo que se va a llenar con caracteres en orden ascendente
// n: La longitud del arreglo que determina cuántos elementos se inicializarán
void iniciar_arreglo_ascendente(char arreglo[], int n) {
    // Genera un número aleatorio para el primer elemento (entre 32 y 126)
    arreglo[0] = (char)(rand() % (126 - 32 + 1) + 32);

    // Asigna los siguientes elementos en orden ascendente
    for (int i = 1; i < n; i++) {
        arreglo[i] = arreglo[i - 1] + 1;  // Siguiente carácter en el orden ascendente
    }
}

// Función que inicializa los elementos de un arreglo en orden descendente
// arreglo[]: El arreglo que se va a llenar con caracteres en orden descendente
// n: La longitud del arreglo que determina cuántos elementos se inicializarán
void iniciar_arreglo_descendente(char arreglo[], int n) {
    // Genera un número aleatorio para el primer elemento (entre 32 y 126)
    arreglo[0] = (char)(rand() % (126 - 32 + 1) + 32);

    // Asigna los siguientes elementos en orden descendente
    for (int i = 1; i < n; i++) {
        arreglo[i] = arreglo[i - 1] - 1;  // Siguiente carácter en el orden descendente
    }
}
