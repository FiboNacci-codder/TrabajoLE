...
// Declaración de una función externa en ensamblador que implementa el algoritmo Quicksort.
// La función quicksort toma como parámetros:
// - Un puntero `char* arr` que apunta al inicio del array a ordenar,
// - Un entero `int start` que indica el índice de inicio del array,
// - Un entero `int end` que indica el índice final del array.
// La función retorna un puntero a `int` que corresponde a la dirección del array ya ordenado.
extern int* quicksort(char* arr, int start, int end);
